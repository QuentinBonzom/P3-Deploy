...
function Recettes() {
  ...
  <section className="flex justify-around pb-3 max-md:hidden text-lg">
    <button className="rounded-full border-2 p-2">Catégorie</button>
    <button className="rounded-full border-2 p-2">Regime</button>
    <button className="rounded-full border-2 p-2">Difficulté</button>
    <button className="rounded-full border-2 p-2 bg-primary/80">Réinitialiser les filtres</button>
  </section>
      ...
      <Link to="/Details" key={recipe.id}>
        <article className="flex bg-white rounded-xl shadow-md overflow-hidden max-h-60">
          <div className="bg-primary/80 items-center justify-center w-30 h-54 md:w-60 md:h-56 lg:w-7xl">
            ...
            <p className="text-sm text-center">{renderStars(recipe.rate)}</p>
            <p className="text-xs text-center">{recipe.diet_name}</p>
            <p className="text-sm font-semibold text-center">{recipe.difficulty}</p>
            </div>
          </div>

          <div className="p-3 flex-1 flex flex-col justify-between">
            <div>
              <span className="font-bold text-lg text-center">{recipe.recipe_name}</span>
              <p className="truncate w-[190px] mb-2">{recipe.description}</p>
              <p className="text-sm">Préparation : {recipe.time_preparation} min</p>
              <p className="text-sm mb-3">Environ : {recipe.kcal} Kcal/Pers.</p>
            </div>
